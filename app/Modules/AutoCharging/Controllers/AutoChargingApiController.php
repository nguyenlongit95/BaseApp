...
class AutoChargingApiController extends FrontendController
{
    ...
    public function postApiAutoCharging(Request $request) {
    ...
    private function checkUser($postdata){
        ...
        $idMerchant = $merchant->user;
        $wallet_num = $merchant->wallet_num;
        ...
        $wallet = Wallet::where('number', $wallet_num)->first();
        if(!$wallet){
            $result['status']   = 70;
    ...
    private function checkSign($params){
        ...
        $system_sign = md5($merchant->partner_id.$merchant->partner_key.$params['telco'].$params['code'].$params['serial'].$params['request_id']);
