                        ...
                        <form action="" method="POST" >
                            <div class="card-body row">
                                <table class="table table-bordered table-striped">
                                    ...
                                    @foreach( $telcos as $telco )
                                        <tr>
                                            <td>{{ $telco->name  }} Fee:</td>

                                            @foreach( $groups as $group )
                                                <td><input type="number" class="form-control FeesAjax" data-telco="{{ $telco->key }}" data-group="{{ $group->id }}" data-key="fees" value="{{ $fees->getValueByGroupandTelco($group->id, $telco->key,'fees') }}" /></td>
                                            @endforeach
                                        </tr>
                                        <tr>
                                            <td>{{ $telco->name  }} Penalty:</td>
                                            @foreach( $groups as $group )
                                                <td><input type="number" class="form-control FeesAjax" data-telco="{{ $telco->key }}" data-group="{{ $group->id }}" data-key="penalty" value="{{ $fees->getValueByGroupandTelco($group->id,$telco->key,'penalty') }}" /></td>
                                            @endforeach
                                        </tr>
                                    @endforeach
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $(".FeesAjax").on('input propertychange change', function(){
                $.ajax({
                    url: "{{ url('/').'/'.$backendUrl.'/chargings/update-fees' }}",
                    type : "post",
                    dataType:"text",
                    data : {
